...
class ErrorHandler extends ApplicationPart {
    ...
    protected function handleException($exception) {
        $app = Base::a();
        if ($app instanceof Web\WebApplication) {
    ...
    protected function renderException() {
        ...
        if ($exception instanceof HttpException || !APP_DEBUG)
            $this->renderError();
        else {
            if ($this->isAjax())
                Base::a()->displayException($exception);
            else
    ...
    protected function renderError() {
        if ($this->errorAction !== null)
            Base::a()->runController($this->errorAction);
        else {
            $data = $this->getError();
            if ($this->isAjax())
                Base::a()->displayError($data['code'], $data['message'], $data['file'], $data['line']);
            elseif (APP_DEBUG)
    ...
    protected function getViewFile($view, $code) {
        $viewPaths = array(
            null,
            null,
            APP_PATH . DIRECTORY_SEPARATOR . 'views',
        );
    ...
    protected function getVersionInfo() {
        if (APP_DEBUG) {
            $version = 'DAPS Toolkit / ' . Base::getVersion();
